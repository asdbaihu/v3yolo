...
class AttributeNetwork(nn.Module):
    def __init__(self, hidden_size=256, output_size=128):
        super().__init__()
        input_size=768
...
class YOLOLayer(nn.Module):
    def __init__(self, anchors, nc, img_size, yolo_index):
        ...
        self.CN = ConfNetwork(self.num_ft)
    ...
    def forward(self, p, img_size, emb_ft, var=None):
        '''
        p -> fc -> v_emb      RN -> cls prediction
                      +   ->  
             s_emb -> AN      xyxy,conf
        '''
        ...
        # (bs,512,13,13) -> (bs,13,13,256) -> (bs,13,13,128*3) -> (bs,13,13,3,128) -> (bs,13,13,3*80,128)
        v_emb = p.permute(0, 2, 3, 1).contiguous()  # prediction
        v_emb = self.fc(v_emb)
        v_emb = v_emb.view(bs, self.ny, self.nx, self.na, self.num_ft)
        v_emb_ = v_emb.repeat(1,1,1,self.nc,1)
        
        ### relation network ###
        # (80,768) -> (80,128) -> (bs,13,13,3*80,128)
        s_emb = self.AN(emb_ft)
        s_emb = s_emb.view(1,1,1,self.nc, self.num_ft)
        s_emb = s_emb.repeat(bs,self.ny, self.nx, self.na, 1)
...
class Darknet(nn.Module):
    ...
    def forward(self, x, emb, var=None):
        img_size = x.shape[-2:]
        layer_outputs = []
        output = []
